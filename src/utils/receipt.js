...
export const buildReceiptHtml = (group, accountEmail, eventName) => {
  ...
  const cleanEventName = eventName && eventName.trim();
  const title = cleanEventName || 'Comprovante de Inscrição';
...
<body>
  <div class="receipt">
    ...
    <div class="note">
      Comprovante interno de inscrição${cleanEventName ? ` de ${escapeHtml(cleanEventName)}` : ''}. Este documento NÃO é uma nota fiscal.
    </div>
  </div>
</body>
</html>`;
...
export const printReceipt = (group, accountEmail, eventName) => {
  const html = buildReceiptHtml(group, accountEmail, eventName);
